...
import { generateDigitSumProblem, type DigitSumProblem } from '../../utils/problemGenerator';
import DifficultySelector from '../shared/DifficultySelector';
import RoundFeedback from '../shared/RoundFeedback';

type Phase = 'config' | 'playing' | 'feedback' | 'results';

const DIFF_PARAMS: Record<Difficulty, { maxDigits: number }> = {
  easy: { maxDigits: 3 },
  medium: { maxDigits: 6 },
  hard: { maxDigits: 10 },
};
...
interface RoundResult {
  problem: DigitSumProblem;
  userAnswer: number | null;
  correct: boolean;
}

export default function DigitSumChain({
  worksheetMode,
  onComplete,
}: {
  ...
  const [allProblems, setAllProblems] = useState<DigitSumProblem[]>(() =>
    worksheetMode
      ? Array.from({ length: worksheetMode.rounds }, () =>
          generateDigitSumProblem(DIFF_PARAMS[(worksheetMode.difficulty ?? 'medium') as Difficulty].maxDigits)
        )
      : []
  );
  ...
  const startGame = () => {
    const ps = Array.from({ length: totalRounds }, () =>
      generateDigitSumProblem(DIFF_PARAMS[effectiveDiff].maxDigits)
    );
    setAllProblems(ps);
    setCurrentIdx(0);
    setResults([]);
    ...
    return (
      <div className="max-w-md mx-auto text-center">
        <div className="flex justify-between mb-4 text-sm text-gray-400">
          ...
          <span>Digit sum → digital root</span>
        </div>
        <RoundFeedback
        ...
        >
          <p className="text-gray-300">
            Digital root of {p.number} = {p.answer}
          </p>
          {!lastResult.correct && (
  ...
  if (phase === 'results') {
    ...
    return (
      <div className="max-w-md mx-auto text-center">
        <h2 className="text-2xl font-bold mb-4 text-primary">Digit sum chain</h2>
        ...
        <div className="space-y-2 mb-6 max-h-64 overflow-y-auto text-left">
          {results.map((r, i) => (
            ...
            >
              <span>
                dr({r.problem.number}) = {r.problem.answer}
              </span>
              {!r.correct && (
    ...
    return (
      <div className="max-w-md mx-auto text-center">
        ...
        <div className="bg-surface rounded-2xl p-8">
          <p className="text-gray-400 text-sm mb-4">Digital root (repeat digit sum until one digit)</p>
          <p className="text-4xl font-bold text-primary mb-6 break-all">{problem.number}</p>
          <input
            ref={inputRef}
            type="text"
            inputMode="numeric"
  ...
  return (
    <div className="max-w-md mx-auto">
      <h2 className="text-2xl font-bold mb-6 text-center text-primary">Digit sum chain</h2>
      <div className="bg-surface rounded-xl p-6 space-y-4">
        <p className="text-gray-400 text-sm">Find the digital root (1–9, except single 0 for 0).</p>
