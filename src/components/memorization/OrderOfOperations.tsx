  ...
  if (phase === 'results') {
    ...
    return (
      <div className="max-w-lg mx-auto text-center text-primary">
        <h2 className="text-2xl font-bold mb-4">Order of operations — results</h2>
        ...
        <div className="space-y-2 mb-6 text-left max-h-72 overflow-y-auto">
          {results.map((r, i) => (
            ...
            >
              <div className="font-mono text-gray-300">{r.problem.expression} = {r.problem.answer}</div>
              {!r.correct && <div className="text-gray-400 text-xs mt-1">You: {r.userAnswer || '—'}</div>}
            </div>
          ))}
        </div>
