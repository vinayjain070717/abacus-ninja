...
export default function PowersOf2({
  ...
  if (phase === 'results') {
    ...
    return (
      <div className="max-w-md mx-auto text-center">
        <h2 className="text-2xl font-bold mb-4 text-primary">Powers of 2</h2>
        ...
        <div className="space-y-2 mb-6 max-h-64 overflow-y-auto text-left">
          {results.map((r, i) => (
            ...
            >
              <span>
                {r.problem.mode === 'value-from-exp'
                  ? `2^${r.problem.exponent} = ${r.problem.value}`
                  : `${r.problem.value} = 2^${r.problem.exponent}`}
              </span>
              {!r.correct && (
