...
export default function FibonacciSprint({
  ...
  if (phase === 'results') {
    const score = results.reduce((s, r) => s + r.correctTerms, 0);
    const total = results.reduce((s, r) => s + r.totalTerms, 0);
    return (
      <div className="max-w-md mx-auto text-center text-primary">
        <h2 className="text-2xl font-bold mb-4">Fibonacci sprint</h2>
        <div className="bg-surface rounded-xl p-6 mb-6">
          <div className="text-4xl font-bold mb-2">
            {score}/{total}
          </div>
          <p className="text-gray-400 text-sm">Correct terms across all rounds.</p>
        </div>
        <div className="space-y-2 mb-6 max-h-64 overflow-y-auto text-left">
          {results.map((r, i) => (
            ...
            >
              <div className="text-gray-200">{r.problem.a}, {r.problem.b} → …</div>
              <div className="font-mono text-gray-400 mt-1">
                {r.correctTerms}/{r.totalTerms} terms — expected: {r.problem.sequence.slice(2).join(', ')}
              </div>
            </div>
          ))}
        </div>
