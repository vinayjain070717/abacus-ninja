...
import { generateComplementProblem, type ComplementProblem } from '../../utils/problemGenerator';
import DifficultySelector from '../shared/DifficultySelector';
import RoundFeedback from '../shared/RoundFeedback';

type Phase = 'config' | 'playing' | 'feedback' | 'results';

const DIFF_PARAMS: Record<Difficulty, { target: number }> = {
  easy: { target: 10 },
  medium: { target: 100 },
  hard: { target: 1000 },
};
...
interface RoundResult {
  problem: ComplementProblem;
  userAnswer: number | null;
  correct: boolean;
}

export default function ComplementTo100({
  worksheetMode,
  onComplete,
}: {
  ...
  const [allProblems, setAllProblems] = useState<ComplementProblem[]>(() =>
    worksheetMode
      ? Array.from({ length: worksheetMode.rounds }, () =>
          generateComplementProblem(DIFF_PARAMS[(worksheetMode.difficulty ?? 'medium') as Difficulty].target)
        )
      : []
  );
  ...
  const startGame = () => {
    const ps = Array.from({ length: totalRounds }, () =>
      generateComplementProblem(DIFF_PARAMS[effectiveDiff].target)
    );
    setAllProblems(ps);
    setCurrentIdx(0);
    setResults([]);
    ...
    return (
      <div className="max-w-md mx-auto text-center">
        <div className="flex justify-between mb-4 text-sm text-gray-400">
          ...
          <span>Complement to {p.target}</span>
        </div>
        <RoundFeedback
  ...
  if (phase === 'results') {
    ...
    return (
      <div className="max-w-md mx-auto text-center">
        <h2 className="text-2xl font-bold mb-4 text-primary">Complement drill</h2>
        ...
        <div className="space-y-2 mb-6 max-h-64 overflow-y-auto text-left">
          {results.map((r, i) => (
            ...
            >
              <span>
                {r.problem.target} − {r.problem.number} = {r.problem.answer}
              </span>
              {!r.correct && (
    ...
    return (
      <div className="max-w-md mx-auto text-center">
        ...
        <div className="bg-surface rounded-2xl p-8">
          <p className="text-gray-400 text-sm mb-4">What do you add to reach {problem.target}?</p>
  ...
  return (
    <div className="max-w-md mx-auto">
      <h2 className="text-2xl font-bold mb-6 text-center text-primary">Complement drill</h2>
      <div className="bg-surface rounded-xl p-6 space-y-4">
        <p className="text-gray-400 text-sm">Find what you add to reach 10, 100, or 1000 (by difficulty).</p>
