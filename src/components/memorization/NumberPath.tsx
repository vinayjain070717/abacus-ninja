...
export default function NumberPath({
  ...
  if (phase === 'results') {
    const score = results.reduce((s, o) => s + o.correctCells, 0);
    const total = results.reduce((s, o) => s + o.totalCells, 0);
    return (
      <div className="max-w-md mx-auto text-center text-primary">
        <h2 className="text-2xl font-bold mb-4">Number path</h2>
        <div className="bg-surface rounded-xl p-6 mb-6 border border-gray-700/50">
          <div className="text-4xl font-bold mb-2">
            {score}/{total}
          </div>
          <p className="text-gray-400 text-sm">Correct cells</p>
        </div>
